...
#ifdef OVERWORLD
    vec3 skyColorSqrt = vec3(0.46,0.66 , 1* 1);

    vec3 noonUpSkyColor     = pow(skyColor, vec3(1.32)) * 1;
    vec3 noonMiddleSkyColor = skyColorSqrt ;
    vec3 noonDownSkyColor   = noonUpSkyColor *vec3(1,1,1);
